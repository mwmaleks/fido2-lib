...
function parseAuthenticatorData(authnrDataArrayBuffer, { withAttestation = true } = {}) {
    ...
    // we do not invoke aaguid and other validations when validate assertion
    // so skip this block if withAttestation set to false
    if (withAttestation && attestation) {
...
function parseAuthnrAssertionResponse(msg) {
    ...
    let ret = new Map([
        ...
        ...parseAuthenticatorData(msg.response.authenticatorData, { withAttestation: false })
    ]);
